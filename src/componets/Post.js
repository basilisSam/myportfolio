...
export default function Post() {
  const [postDatam, setPost] = useState(null);
  ...
  return (
    <main className='bg-green-100 min-h-screen p-12'>
      <section className='container mx-auto'>
        ...
        <h2 className='text-lg text-gray-600 flex justify-center mb-12 font-mono'>
          Welcome to my page of blog posts 🙌
        </h2>
        <div className='grid md:-cols-2 lg:grid-cols-3 gap-8'>
          {postDatam &&
            postDatam.map((post, index) => (
              <article>
                <Link to={"/post/" + post.slug.current} key={post.slug.current}>
                  <span
                    className='block h-64 relative rouded shadow leading-snug bg-white border-l-8 border-green-400'
                    key={index}
                  >
                    <img
